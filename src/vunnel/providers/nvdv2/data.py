                ...
                if char == "\\":  # an escaped character, percent encode it if possible
                    if pos != (len(element) - 1):  # check the next character and transform into percent encoded string
                        ...
                        if encoded:
                            ...
                            logging.warn("No encoding found for {}{}".format(char, n_char))
                            result += char + n_char
                    else:  # this is the last char, nothing to percent encode
                        logging.warn("{} is the last char, skipping percent encoded transformation".format(char))
                        result += char
                        ...
                        if any(
                            ...
                            if not (base_cpe.vendor, base_cpe.product) in affected_tuple_set:
                                ...
                                for aver in affected_versions:
                                    ...
                                    # cpe22_uri, _ = fs_uri_cpe_tup_dict.get(aver_cpe.as_cpe23_fs(), (None, None))
                                    # if not cpe22_uri:
                                    #     cpe22_uri = aver_cpe.as_cpe22_uri()
                                    #
                                    # cpe_set.add(cpe22_uri)
...
# def refresh(workspace, timeout, skip_if_exists):
#     global namespace
#     data_provider = NVDDataProvider(workspace=workspace, download_timeout=timeout)
#     group = "{}:cves".format(namespace)

#     for cve_id, cve in data_provider.get(skip_if_exists):
...
#     # load config override for requests_timeout if it exists
#     timeout_override = None
#     if config and config.get("requests_timeout", None):
#         try:
#             timeout_override = int(config.get("requests_timeout"))
#             logger.debug(
...
#     provider = NVDDataProvider(workspace='/tmp/nvdv2')
#
#     xml_file_path = '/tmp/nvdv2/official-cpe-dictionary_v2.3.xml'
#     print('Normalize cpe dictionary from {}'.format('/tmp/nvdv2/official-cpe-dictionary_v2.3.xml'))
#     fs_cpe_dict = provider._normalize_cpe_dictionary(xml_file_path)
#     print('CPE dictionary contains {} records'.format(len(fs_cpe_dict)))
#
#     count = 0
#
#     # Normalize data
#     json_file_path = '/tmp/nvdv2/nvd-2019.json'
#     print('normalize json file {}'.format(json_file_path))
#     for cve_id, cve in provider._normalize(json_file_path, fs_cpe_dict):
#         count += 1
#         print(cve_id)
#         if count % 1000 == 0:
...
#
#     # free up the memory held by this giant dictionary
#     print('Size of cpe dictionary before clear() {}'.format(sys.getsizeof(fs_cpe_dict)))
#     fs_cpe_dict.clear()
#     print('Size of cpe dictionary after clear() {}'.format(sys.getsizeof(fs_cpe_dict)))
#     fs_cpe_dict = None
#
#
