...
class Parser:
    def __init__(self, ws: Workspace, logger: logging.Logger | None = None, rocky_linux_versions: list[str] | None = None):
        if rocky_linux_versions is None:
            rocky_linux_versions = ["8", "9"]
        self.rocky_linux_versions = rocky_linux_versions
        self.workspace = ws
        if not logger:
