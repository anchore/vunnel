...
class Store(Strategy):
    ...
    def _ensure_index(
        ...
        if self._cpe_index is None or self._pkg_index is None:
            with self._index_lock:
                if self._cpe_index is None or self._pkg_index is None:
                    self._build_index()
        cpe_index = self._cpe_index
        pkg_index = self._pkg_index
        if cpe_index is None or pkg_index is None:
            raise RuntimeError("index build failed: indexes are not populated")
        return cpe_index, pkg_index
    ...
    def get(
        ...
        if cpe_or_package.lower().startswith("cpe:"):
            ...
            candidates = list(cpe_index.get(cpe_key, []))
            # also try v6 simplified CPE format since grype-db may store CPEs this way
            v6_cpe = cpe_to_v6_format(cpe_or_package)
            if v6_cpe:
                v6_key: tuple[str, str] = (vuln_id.lower(), v6_cpe.lower())
                candidates += cpe_index.get(v6_key, [])
        else:
