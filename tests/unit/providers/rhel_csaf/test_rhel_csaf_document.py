...
def test_vuln_basic_data(csaf_filename):
    ...
    for vuln in results:
        assert vuln.get("item", {}).get("Vulnerability", {}).get("Name") == doc.cve_id
        assert vuln.get("item", {}).get("Vulnerability", {}).get("Severity") == doc.severity
        assert vuln.get("item", {}).get("Vulnerability", {}).get("Link") == doc.vuln_url
