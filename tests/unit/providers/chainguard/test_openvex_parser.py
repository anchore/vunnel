...
class TestOpenVEXParser:
    ...
    def test_normalize(self, openvex_parser):
        ...
        assert {'foo': foo, 'bar': bar} == openvex_parser._normalize("rolling", test_data)
...
def test_openvex_parser_via_snapshot(helpers, disable_get_requests, monkeypatch):
    ...
    assert d == {
        'CVE-2007-2728': {'vulnerability': {'name': 'CVE-2007-2728'}, 'products': [{'@id': 'pkg:pypi/chainguard/php@0', 'identifiers': {'purl': 'pkg:pypi/chainguard/php@0'}}], 'status': 'fixed'},
        'CVE-2023-27898': {'vulnerability': {'name': 'CVE-2023-27898'}, 'products': [{'@id': 'pkg:pypi/chainguard/jenkins@2.394-r0', 'identifiers': {'purl': 'pkg:pypi/chainguard/jenkins@2.394-r0'}}], 'status': 'fixed'}
    }
